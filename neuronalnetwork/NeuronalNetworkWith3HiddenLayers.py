...
class NeuralNetworkWith3HiddenLayers:
    def __init__(self):
        self.x = None
        self.y = None
        self.theta_1 = None
    ...
    def initialize_thetas(self):
        ...
        self.theta_4 = np.random.rand(self.layer_5, (self.layer_4 + 1)) * 2 * self.epsilon - self.epsilon
    ...
    def cost_function(self, t):
        ...
        t3 = np.reshape(t[self.layer_2 * (self.layer_1 + 1) + self.layer_3 * (self.layer_2 + 1) : self.layer_2 * (self.layer_1 + 1) + self.layer_3 * (self.layer_2 + 1) + self.layer_4 * (self.layer_3 + 1)], (self.layer_4, (self.layer_3 + 1)))
        t4 = np.reshape(t[self.layer_2 * (self.layer_1 + 1) + self.layer_3 * (self.layer_2 + 1) + self.layer_4 * (self.layer_3 + 1):], (self.layer_5, (self.layer_4 + 1)))
        m, n = self.x.shape
    ...
    def train(self, destiny):
        ...
        self.theta_3 = np.reshape(theta_optimo[self.layer_2 * (self.layer_1 + 1) + self.layer_3 * (self.layer_2 + 1) : self.layer_2 * (self.layer_1 + 1) + self.layer_3 * (self.layer_2 + 1) + self.layer_4 * (self.layer_3 + 1)], (self.layer_4, (self.layer_3 + 1)))
        self.theta_4 = np.reshape(theta_optimo[self.layer_2 * (self.layer_1 + 1) + self.layer_3 * (self.layer_2 + 1) + self.layer_4 * (self.layer_3 + 1):], (self.layer_5, (self.layer_4 + 1)))
        ...
        arch.create_dataset("Theta4", data=self.theta_4)
    ...
    def predict(self, image):
        ...
        a3 = np.concatenate([np.ones((a3.shape[0], 1)), a3], axis=1)
        a4 = self.sigmoid(a3.dot(self.theta_3.T))
        a4 = np.concatenate([np.ones((a4.shape[0], 1)), a4], axis=1)
        a5 = self.sigmoid(a4.dot(self.theta_4.T)).T
        return a5.argmax(), a5[a5.argmax()]
    ...
    def load_data(self, file):
        ...
        self.theta_4 = loaded_file["Theta4"][:]
