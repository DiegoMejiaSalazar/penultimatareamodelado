...
for x,y in zip(testing_data["X"][:], testing_data["y"][:]):
    descriptor = hog(x, orientations=9, pixels_per_cell=(10,10), cells_per_block=(1, 1))
    predicted_result, _ = r.predict(descriptor.reshape(1, -1))
    predicted_result2, _ = r2.predict(descriptor.reshape(1, -1))
    predicted_result3, _ = r3.predict(descriptor.reshape(1, -1))
